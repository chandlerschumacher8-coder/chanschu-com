  ...
  days.forEach(function(dayStr){
    ...
    noteSlots.forEach(function(slot,colIdx){
      slot.forEach(function(item){
        ...
        ev.style.cursor='grab';
        ev.onclick=function(e){if(_delDragMoved)return;e.stopPropagation();delOpenNoteDetail(n.id);};
        ev.addEventListener('mousedown',(function(nId,evRef,nObj){return function(e){if(e.button!==0)return;delStartNoteDrag(e,nId,evRef,nObj);};})(n.id,ev,n));
        col.appendChild(ev);
      });
    });
  });
...
// Drag & Drop
var _delDragType='delivery';
function delStartNoteDrag(e,id,el,nObj){
  e.preventDefault();_delDragId=id;_delDragEl=el;_delDragging=true;_delDragMoved=false;_delDragType='note';
  _delDragOffsetY=e.clientY-el.getBoundingClientRect().top;
  _delDragGhost=document.createElement('div');_delDragGhost.className='del-drag-ghost';
  _delDragGhost.style.background=el.style.background||'#dcfce7';_delDragGhost.style.borderLeft=el.style.borderLeft||'3px solid #15803d';_delDragGhost.style.color=el.style.color||'#14532d';
  _delDragGhost.textContent=nObj.title+(nObj.time?' · '+nObj.time:'');
  _delDragGhost.style.left=e.clientX+'px';_delDragGhost.style.top=e.clientY+'px';
  document.body.appendChild(_delDragGhost);el.classList.add('dragging');
  document.addEventListener('mousemove',delOnDragMove);document.addEventListener('mouseup',delOnDragEnd);
}
