// api/dc-jobs-get.js — Returns all DC Appliance service jobs
...
export default async function handler(req, res) {
  ...
  try {
    if (useSupabase()) {
      ...
      const { data: counter } = await sb.from('counters').select('value').eq('store_id', store_id).eq('key', 'next_job_id').single();
      ...
      return res.status(200).json({ ok: true, jobs, nextId: counter?.value || jobs.length + 1 });
    }

    // Redis fallback
    const raw = await getRedis().get('dc-service-jobs');
